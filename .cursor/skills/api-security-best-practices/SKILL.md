...
description: "Implement and review endpoint-level API security: authentication, authorization, validation, rate limiting, and OWASP API protections. Use after security requirements are defined."
risk: unknown
source: community
...
- Use when implementing or reviewing a specific REST, GraphQL, or WebSocket endpoint.
- Use when adding endpoint authentication, authorization, schema validation, rate limiting, security headers, or safe error handling.
- Use when remediating an API-specific finding from `security-auditor`.

## Do Not Use This Skill When

- The task is threat modeling, SDLC/CI compliance, incident response, or a broad security assessment; use `security-auditor`.
- The task is storing, rotating, injecting, or granting access to credentials; use `secrets-management`.
- The task is unrelated to an API boundary.
